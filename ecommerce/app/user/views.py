...
from utils.helper import generate_otp, get_object_or_none
...
class PasswordResetRequestView(APIView):
    ...
    def post(self, request):
        ...
        if not user:
            return Responder.error_response(401, errors={"error": "User not found."})
...
class ResetPasswordConfirmView(APIView):
    ...
    def post(self, request):
        ...
        user = get_object_or_none(User, email=email)
        if not user:
            return Responder.error_response(405, errors={"error": "User not found."})

        if not PasswordResetOTP.objects.is_valid_otp(user, otp):
            return Responder.error_response(404, errors={"error": "Invalid OTP."})
...
class AddressListCreateAPIView(APIView):
    ...
    def get(self, request):
        addresses = Address.objects.for_user(request.user)
